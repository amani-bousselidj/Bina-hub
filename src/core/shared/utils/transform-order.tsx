...
// Overloads for array and single order
...
export function formatOrder<T = any>(
  ...
  const orders = [...(isArray ? (order as T[]) : [order as T])]
  ...
  const result = orders.map((order) => {
    ...
    if (isRelatedEntity) {
      if (!(order as any).order) {
        // Instead of returning undefined, return the original order as T
        return createRawPropertiesFromBigNumber(
          options?.includeTotals ? decorateCartTotals(order as unknown as DecorateCartLikeInputDTO) : order
        ) as T
      }
      mainOrder = (order as any).order
    }

    (mainOrder as any).items = (mainOrder as any).items?.map((orderItem: any) => {
    ...
    if (isRelatedEntity) {
      if ((order as any).return) {
        formatOrderReturn((order as any).return, mainOrder)
      }
    ...
    if ((order as any).shipping_methods) {
      (order as any).shipping_methods = (order as any).shipping_methods?.map((shippingMethod: any) => {
    ...
    if ((mainOrder as any).summary) {
      (mainOrder as any).summary = (mainOrder as any).summary?.[0]?.totals
    }

    // Always use mainOrder for decorateCartTotals and createRawPropertiesFromBigNumber
    return createRawPropertiesFromBigNumber(
      options?.includeTotals ? decorateCartTotals(mainOrder as unknown as DecorateCartLikeInputDTO) : mainOrder
    ) as T
  })
