...
export default function Loading() {
  ...
  // Auto-transition through loading steps then show platform features
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentStep(prev => {
        if (prev < steps.length - 1) return prev + 1;
        // Switch to platform showcase after loading completes
        setTimeout(() => setLoadingPhase("showcase"), 1000);
        return prev;
      });
    }, 800);
  ...
  // Loading Phase - Show animated loading steps
        ...
        <div className="relative z-10 text-center space-y-8 max-w-lg mx-auto">
          ...
          <div className="flex justify-center mb-8">
            <div className="relative">
              <div className="w-24 h-24 bg-gradient-to-r from-blue-600 via-purple-600 to-blue-800 rounded-3xl flex items-center justify-center shadow-2xl">
                <span className="text-white font-bold text-4xl">بـ</span>
              </div>
              <div className="absolute -inset-2 bg-gradient-to-r from-blue-400 to-purple-400 rounded-3xl opacity-30 animate-pulse"></div>
            </div>
          </div>
          ...
          <div className="flex justify-center mb-8">
            <div className="relative">
              <div className="w-16 h-16 animate-spin rounded-full border-4 border-blue-200 border-t-blue-600"></div>
              <div className="absolute inset-2 animate-spin rounded-full border-3 border-purple-200 border-r-purple-600" style={{animationDuration: '1.5s', animationDirection: 'reverse'}}></div>
              <div className="absolute inset-4 animate-pulse rounded-full bg-gradient-to-r from-blue-400 to-purple-400 opacity-30"></div>
            </div>
          </div>

          {/* Loading Steps */}
          <div className="space-y-4">
            <h2 className="text-3xl font-bold text-gray-800 mb-6">منصة بنّا التجارية</h2>
            {steps.map((step, index) => (
              <div
                key={index}
                className={cn(
                  "text-lg transition-all duration-500 px-4 py-2 rounded-lg",
                  index <= currentStep 
                    ? "text-blue-600 font-semibold bg-blue-50 transform scale-105" 
            ...
            {/* Progress Bar */}
            <div className="w-full max-w-md h-3 bg-gray-200 rounded-full mx-auto mt-6 overflow-hidden">
              <div 
                className="h-full bg-gradient-to-r from-blue-500 via-purple-500 to-blue-600 rounded-full transition-all duration-500"
  ...
  // Platform Showcase Phase - Show comprehensive features
  return (
