...
import { constructionGuidanceService, ConstructionPhase } from '@/services/constructionGuidanceService';
...
export default function BuildingAdvicePage() {
  ...
  useEffect(() => {
    if (projectType) {
      // Mock data for construction phases since getProjectPhases doesn't exist
      const mockPhases: ConstructionPhase[] = [
        { id: 'planning', name: 'التخطيط', description: 'مرحلة التخطيط والتصميم', status: 'pending' },
        { id: 'foundation', name: 'الأساسات', description: 'أعمال الحفر والأساسات', status: 'pending' },
        { id: 'structure', name: 'الهيكل', description: 'بناء الهيكل الإنشائي', status: 'pending' }
      ];
      setConstructionPhases(mockPhases);
    }
  }, [projectType]);
                        ...
                        return (
                          <div key={phase.id} className={`border rounded-lg p-4 cursor-pointer transition-all ${
                            ...
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <Badge variant="outline">مرحلة {index + 1}</Badge>
                            </div>
                            {isActive && advice && (
